...
void getDictionary(vector<string>& palabras) {
    ifstream archivo("inputs/test_dict.txt");
    if (!archivo) {
...
void escribirParte(int thread_id, size_t bytes_por_thread, vector<string>& palabras) {
    string nombre_archivo = "outputs/test/parts/parte_" + to_string(thread_id) + ".txt";
...
void unirArchivos(int num_threads, const std::string& archivo_final) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        std::string nombre_parte = "outputs/test/parts/parte_" + std::to_string(i) + ".txt";
...
int main() {
    ...
    unirArchivos(num_threads, "outputs/test/archivo_1GB.txt");
